...
<template>
   <div class="px-0">
      ...
      <div class="row m-0 p-0 mt-3">
         <p style="font-size: 32px" class="m-0 p-0 fw-light text-color3">{{ user.firstname }}&nbsp;{{ user.lastname }}</p>
      </div>
      ...
      <div class="row m-0 p-0 mt-2">
         <div class="m-0 p-0 d-flex text-truncate d-inline-flex justify-content-start align-items-center">
            ...
            <p style="font-size: 15px" class="m-0 p-0 ms-2 fw-normal text-color4">{{ user.email }}</p>
         </div>
      </div>
      ...
      <div class="row m-0 p-0 mt-1">
         <div class="m-0 p-0 d-flex d-inline-flex justify-content-start align-items-center">
            ...
            <p style="font-size: 15px" class="m-0 p-0 ms-2 fw-normal text-color4">{{ user.username }}</p>
      ...
      <div class="row m-0 p-0 mt-1">
         <div class="m-0 p-0 text-truncate d-flex d-inline-flex justify-content-start align-items-center">
            ...
            <p style="font-size: 15px" class="m-0 p-0 ms-2 fw-normal text-color4">Calificaciones: 0</p>
         </div>
      </div>
      ...
      computed: {
         ...
         correctedImageUrl(){
            if(this.user.profile_picture != null){
               return axios.defaults.baseURL.replace("/api", "") + this.user.profile_picture.url.replace("public/", "storage/")
            }else{
               return (this.user.gender == "masculino")
